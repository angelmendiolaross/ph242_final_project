...
## Comment B: I think this is to delete, right?
```
...
for (V in 1:10) {
  ...
  EstA=glm(infection_rate~population+population_density+white+asian_american+hispanic+median_hh_income+pct_unemployed+pct_college_associate+A, data=train, family='gaussian')
  ...
  EstC=glm(infection_rate~population+population_density+white+asian_american+hispanic+median_hh_income+pct_unemployed+pct_college_associate+populationsq+population_densitysq+A, data=train, family='gaussian')
 
  EstD=glm(infection_rate~population+population_density+african_american+asian_american+hispanic+median_hh_income+pct_unemployed+pct_college_associate+populationsq+population_densitysq+A, data=train, family='gaussian')
...
for (V in 1:10) {
  ...
  ## using glm to fit each algorithm on the training set
  Est_Unadj=glm(infection_rate~A, data=train, family='gaussian')
...
```{r}
## 2 vs 1! ##
IPTW1=mean(wt1*as.numeric(ObsData$A==2)*ObsData$infection_rate)-mean(wt1*as.numeric(ObsData$A==1)*ObsData$infection_rate)

HT1=(mean(wt1*as.numeric(ObsData$A==2)*ObsData$infection_rate)/mean(wt1*as.numeric(ObsData$A==2)))-(mean(wt1*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt1*as.numeric(ObsData$A==1)))

IPTW2=mean(wt2*as.numeric(ObsData$A==2)*ObsData$infection_rate)-mean(wt2*as.numeric(ObsData$A==1)*ObsData$infection_rate)

HT2=(mean(wt2*as.numeric(ObsData$A==2)*ObsData$infection_rate)/mean(wt2*as.numeric(ObsData$A==2)))-(mean(wt2*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt2*as.numeric(ObsData$A==1)))

IPTW3=mean(wt3*as.numeric(ObsData$A==2)*ObsData$infection_rate)-mean(wt3*as.numeric(ObsData$A==1)*ObsData$infection_rate)

HT3=(mean(wt3*as.numeric(ObsData$A==2)*ObsData$infection_rate)/mean(wt3*as.numeric(ObsData$A==2)))-(mean(wt3*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt3*as.numeric(ObsData$A==1)))

IPTW4=mean(wt4*as.numeric(ObsData$A==2)*ObsData$infection_rate)-mean(wt4*as.numeric(ObsData$A==1)*ObsData$infection_rate)

HT4=(mean(wt4*as.numeric(ObsData$A==2)*ObsData$infection_rate)/mean(wt4*as.numeric(ObsData$A==2)))-(mean(wt4*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt4*as.numeric(ObsData$A==1)))
...
## 1 vs 0! ##
IPTW1=mean(wt1*as.numeric(ObsData$A==1)*ObsData$infection_rate)-mean(wt1*as.numeric(ObsData$A==0)*ObsData$infection_rate)

HT1=(mean(wt1*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt1*as.numeric(ObsData$A==1)))-(mean(wt1*as.numeric(ObsData$A==0)*ObsData$infection_rate)/mean(wt1*as.numeric(ObsData$A==0)))

IPTW2=mean(wt2*as.numeric(ObsData$A==1)*ObsData$infection_rate)-mean(wt2*as.numeric(ObsData$A==0)*ObsData$infection_rate)

HT2=(mean(wt2*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt2*as.numeric(ObsData$A==1)))-(mean(wt2*as.numeric(ObsData$A==0)*ObsData$infection_rate)/mean(wt2*as.numeric(ObsData$A==0)))

IPTW3=mean(wt3*as.numeric(ObsData$A==1)*ObsData$infection_rate)-mean(wt3*as.numeric(ObsData$A==0)*ObsData$infection_rate)

HT3=(mean(wt3*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt3*as.numeric(ObsData$A==1)))-(mean(wt3*as.numeric(ObsData$A==0)*ObsData$infection_rate)/mean(wt3*as.numeric(ObsData$A==0)))

IPTW4=mean(wt4*as.numeric(ObsData$A==1)*ObsData$infection_rate)-mean(wt4*as.numeric(ObsData$A==0)*ObsData$infection_rate)

HT4=(mean(wt4*as.numeric(ObsData$A==1)*ObsData$infection_rate)/mean(wt4*as.numeric(ObsData$A==1)))-(mean(wt4*as.numeric(ObsData$A==0)*ObsData$infection_rate)/mean(wt4*as.numeric(ObsData$A==0)))
...
st.MSM1=glm(infection_rate ~ A+population+population_density+white+asian_american+hispanic+median_hh_income+pct_unemployed+pct_college_associate, data=ObsData, weights=wt.MSM1)
...
st.MSM2=glm(infection_rate ~ A+population+population_density+african_american+asian_american+hispanic+median_hh_income+pct_unemployed+pct_college_associate, data=ObsData, weights=wt.MSM2)
...
st.MSM3=glm(infection_rate ~ A+population+population_density+white+asian_american+hispanic+median_hh_income+pct_unemployed+pct_college_associate+populationsq+population_densitysq, data=ObsData, weights=wt.MSM3)
...
st.MSM4=glm(infection_rate ~ A+population+population_density+african_american+asian_american+hispanic+median_hh_income+pct_unemployed+pct_college_associate+populationsq+population_densitysq, data=ObsData, weights=wt.MSM4)
...
## Needs to be binary - use these results, then redo G-Comp and IPTW with binary only ##

### ONLY UNADJUSTED HERE - WAITING FOR THIS TO RUN! ###
...
# adjusted 
ltmle.SL_esta=ltmle(df_shortbinary, Anodes='A', Ynodes='Y', abar=1, SL.library=SL.library) 
summary(ltmle.SL_esta)
...
## The IPTW estimate of Ψ(P0) is -29.022%. We can interpret this statistical estimand as the marginal difference in the infection rate associated with mask enforcement, after controlling for measured confounders. If the identifiability assumptions (i.e. randomization and positivity) held, we could then interpret ψ0 in terms of the average treatment effect (a.k.a. the causal risk difference).

## Truncate weights ARBITRARILY at 10 and 50 
sum(wt>5) ## 324 observations
wt.trunc_5 <- wt
wt.trunc_5[wt.trunc_5>5] = 5

sum(wt>50) ## 72 observations
wt.trunc_50 <- wt
...
## Truncated at 5
mean(wt.trunc_5*as.numeric(df_short$A==2)*df_short$infection_rate) - mean(wt.trunc_5*as.numeric(df_short$A==0)*df_short$infection_rate)
## 458.76
...
## Truncation at different levels yields different results, but not radically different. The more observations we eliminate, the higher estimator we get. For instance, if we truncate at weight 5, which means we lose 324 observations, we get an estimator of 458.76%, whereas if we truncate at 500, which means we lose 36 observations, we get an estimator of 385.30%, same as the IPTW value without truncation. 
...
## For this sample of counties, the modified Horvitz-Thompson estimator yielded an estimate of -449.94%. 
```
...
df_shortbinary <- df_shortbinary %>% 
  filter(!is.na(infection_rate), !is.na(population), !is.na(population_density), !is.na(african_american), !is.na(asian_american), !is.na(hispanic), !is.na(median_hh_income), !is.na(pct_college_associate), !is.na(pct_unemployed), !is.na(log_population), !is.na(log_popdensity), !is.na(A_rev), !is.na(log_popdensity)) %>%
  mutate(A=A_rev) %>%
...
summary(df_shortbinary$infection_rate)
## we'll be cutting the variable in half at the median, 157.59.

df_shortbinary$infection_rate_bin <- cut(df_shortbinary$infection_rate, br=c(0.00, 157.59, 11103.32),labels = c("0", "1"))

df_shortbinary <- df_shortbinary %>% filter(!is.na(infection_rate_bin))

# df_shortbinary <- df_shortbinary %>% mutate(
...
df_shortbinary$id <- 1:n

# Function to handcode TMLE with Super Learner
run.tmle <- function(df_shortbinary, SL.library, id=NULL){
  
  # Estimate the conditional mean outcome Qbar(A,W)
  # dataframe X with baseline covariates and exposure
  X <- subset(df_shortbinary, select=c(A, population, population_density, african_american, asian_american, hispanic,median_hh_income, pct_unemployed, pct_college_associate, log_population, log_popdensity))
  ...
  QbarSL <- SuperLearner(Y=df_shortbinary$infection_rate_bin, X=X, SL.library=SL.library, family="binomial", id=id)
  QbarSL
  
  # initial estimates of the outcome, given the observed exposure & covariates
  QbarAW <- predict(QbarSL, newdata=df_shortbinary)$pred
  ...
  gHatSL <- SuperLearner(Y=df_shortbinary$A, X=X, SL.library=SL.library, family="gaussian", id=id)
  ...
  gHatAW <- rep(NA, n)
  gHatAW[df_shortbinary$A==1]<- gHat2W[df_shortbinary$A==1]
  gHatAW[df_shortbinary$A==0]<- gHat0W[df_shortbinary$A==0]

  #-------------------------------------------------
  # Clever covariate H(A,W) for each subject
  #-------------------------------------------------
  H.AW <- as.numeric(df_shortbinary$A==1)/gHat2W - as.numeric(df_shortbinary$A==0)/gHat0W
  ...
  PsiHat.IPTW <- mean(H.AW*df_shortbinary$infection_rate)
  
  #------------------------------------------
  # Update the initial estimator of Qbar_0(A,W)
  #------------------------------------------
  logitUpdate<- glm(df_shortbinary$infection_rate_bin ~ -1 + offset(qlogis(QbarAW)) + H.AW, family='binomial')
  epsilon <- logitUpdate$coef
  QbarAW.star <- plogis(qlogis(QbarAW)+ epsilon*H.AW)
  Qbar1W.star <- plogis(qlogis(Qbar2W)+ epsilon*H.1W)
...
## The TMLE estimate of Ψ(P0) is 12%. After controlling for baseline covariates, the marginal difference in the probability of high infection rate based on some mask enforcement versus no mask enforcement is negative. 
...
for(b in 1:B){
  ...
  bootData <- df_shortbinary[bootIndices,]
  
  # calling the above function
  estimates[b,]<- run.tmle(df_shortbinary=bootData, SL.library=SL.library, id=bootData$id)$estimates
