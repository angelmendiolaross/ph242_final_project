...
$U_{W1}$ to $U_{W4}$ refers to the unmeasured confounders related to the percentage of White, African American, Asian and Hispanic residents at the county level. These could be: economic structure, history of segregation, and political party in the majority. 
...
$[\mathbb{E}(Y|A=1,{W}) - \mathbb{E}(Y|A=0,W)]$**I realized I don't know how to write this for a MSM, but I'll update when I find this information on the class lectures**
...
run.tmle <- function(ObsData, SL.library, id=NULL){
  ...
  X <-subset(ObsData, select=c(population:pct_college_associate))
  ...
  QbarSL<- SuperLearner(Y=ObsData$infection_rate, X=X, SL.library=SL.library, family="gaussian", id=id)
  # QbarSL
  ...
  gHatSL<- SuperLearner(Y=ObsData$A, X=subset(ObsData, select= c(population:asian_american,pct_college_associate)),
                        SL.library=SL.library, family="gaussian", id=id)
  ...
  PsiHat.IPTW <-mean( H.AW*ObsData$infection_rate)
  #------------------------------------------
  # Update the initial estimator of Qbar_0(A,W)
  #------------------------------------------
  logitUpdate<- glm(ObsData$infection_rate ~ -1 +offset(qlogis(QbarAW)) + H.AW, family='gaussian')
...
for(b in 1:B){
  ...
  estimates[b,]<- run.tmle(ObsData=bootData, SL.library=SL.library, id=bootData$id)$estimates
